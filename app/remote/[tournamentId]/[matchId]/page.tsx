...
export default function RemoteControlPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center bg-[#222225]">
      {/* Player 1 */}
      <div className="flex items-center bg-[#2e2e31] rounded-lg p-5 mb-4 w-[600px] max-w-full shadow-md">
        <div className="flex-1">
          <div className="font-bold text-white text-lg">{getCompetitorName(match.competitor1_id)}</div>
        </div>
        {/* Score area */}
        <div className="flex items-center gap-1">
          <span className="text-red-500 font-bold text-base mr-1" style={{ minWidth: 36, textAlign: 'right' }}>Keik</span>
          <button className="bg-blue-600 text-white font-bold text-lg w-8 h-8 rounded ml-1" onClick={() => setScore1(s => s + 1)}>+</button>
          <span className="text-2xl text-white font-mono mx-2" style={{ minWidth: 20, textAlign: 'center' }}>{score1}</span>
          <button className="bg-blue-600 text-white font-bold text-lg w-8 h-8 rounded" onClick={() => setScore1(s => (s > 0 ? s - 1 : 0))}>-</button>
        </div>
      </div>

      {/* Player 2 */}
      <div className="flex items-center bg-[#2e2e31] rounded-lg p-5 mb-6 w-[600px] max-w-full shadow-md">
        <div className="flex-1">
        ...
        <div className="flex items-center gap-1">
          <span className="text-red-500 font-bold text-base mr-1" style={{ minWidth: 36, textAlign: 'right' }}>Keik</span>
          <button className="bg-blue-600 text-white font-bold text-lg w-8 h-8 rounded ml-1" onClick={() => setScore2(s => s + 1)}>+</button>
          <span className="text-2xl text-white font-mono mx-2" style={{ minWidth: 20, textAlign: 'center' }}>{score2}</span>
          <button className="bg-blue-600 text-white font-bold text-lg w-8 h-8 rounded" onClick={() => setScore2(s => (s > 0 ? s - 1 : 0))}>-</button>
        </div>
      </div>

      {/* Timer & Controls */}
      <div className="bg-[#2e2e31] rounded-lg px-10 py-7 w-[600px] max-w-full flex flex-col items-center shadow-md">
